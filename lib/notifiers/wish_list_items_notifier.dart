...
class WishListItemNotifier extends StateNotifier<ViewState> {
  ...
  void _updateProgress(List<WishListItem> wishListItems) async {
    ...
    wishListItems.forEach((wishList) {
      ...
      resultList.add(wishList.edit(
          progress: progress,
          isWishFullfilled: progress == 1.0 ? true : false));
  ...
  Future<void> addAWishListItem(WishListItem wishListItem, String? key) async {
    try {
      state = Loading();
      await wishListRepository.addAWishListItem(wishListItem, key);
      await Future.delayed(Duration(milliseconds: 500), () {
        getAllWishListItems();
      });
    } catch (e) {
  ...
  Future<void> changeCurrency(String currency) async {
    wishListRepository.changeCurrency(currency);
